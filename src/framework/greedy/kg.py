...
class KG:
    ...
    def update_linkpred_info(self, valid1, valid2, test1, test2):
        self.lp_valid1 = set(valid1)
        self.lp_valid2 = set(valid2)
        self.lp_test1 = set(test1)
        self.lp_test2 = set(test2)
        self.lp_valid_all = self.lp_valid1.union(self.lp_valid2)
        self.lp_test_all = self.lp_test1.union(self.lp_test2)

        print()
        print("Number of lp valid examples: {}".format(len(self.lp_valid_all)))
        print("Number of lp test examples: {}".format(len(self.lp_test_all)))
        print()
    ...
    def create_er_vocab(self):
        ...
        for triple in self.relation_triples_set:
            h, r, t = triple
            er_vocab[(h, r)].add(t)
        for triple in self.lp_valid_all:
            h, r, t = triple 
            er_vocab[(h, r)].add(t)
        for triple in self.lp_test_all:
            h, r, t = triple 
            er_vocab[(h, r)].add(t)
    ...
    def add_sup_relation_triples(self, sup_triples):
        """
        only for mode swapping; modes = [mapping, sharing, swapping]
        """
